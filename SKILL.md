...
version: 1.0
description: |
  코드리뷰 — 코드 변경을 개발자가 바로 고칠 수 있는 리뷰 리포트로 만드는 정밀 리뷰 엔진. diff리뷰와 전체감사를 지원하고 버그, 예외, 보안, 회귀, 테스트 누락을 우선 찾는다. 트리거: 코드리뷰, code-review, 코드검토, 리뷰해줘, PR리뷰, diff리뷰, 버그찾아줘, 보안리뷰, 코드품질. NOT: 기획 핸드오프, ruby-skill 단독 승격, 규제·약관·IP 문서.
---
